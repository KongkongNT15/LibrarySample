...
namespace LibrarySample.Pages
{
    ...
    public sealed partial class CLibraryPage : Page, IXml, ICHandler
    {
        ...
        {
            await LibraryPageHelper.ApplyFunctionsAsync(ContentsPanel, XElement, "Functions", "ä÷êî", LibraryType.CLibrary, Category.Function);
        }

        private async Task ApplyFunctionMacros()
        {
            await LibraryPageHelper.ApplyFunctionsAsync(ContentsPanel, XElement, "FunctionMacros", "ä÷êîÉ}ÉNÉç", LibraryType.CLibrary, Category.FunctionMacro);
        }
